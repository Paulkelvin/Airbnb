...
const NavigationItem: FC<NavigationItemWithRouterProps> = ({ menuItem }) => {
  ...
  const renderMainItem = (item: NavItemType) => {
    const isHostCta = item.href === "/add-listing";
    return (
      <Link
        rel="noopener noreferrer"
        className={`inline-flex items-center text-sm xl:text-base py-2 px-4 xl:px-5 rounded-full ${
          isHostCta
            ? "font-medium border border-primary-6000 text-primary-700 dark:text-primary-300 hover:bg-primary-50 dark:hover:bg-primary-900/20"
            : "font-normal text-neutral-700 dark:text-neutral-300 hover:text-neutral-900 hover:bg-neutral-100 dark:hover:bg-neutral-800 dark:hover:text-neutral-200"
        }`}
        href={item.href || "/"}
      >
        {item.name}
