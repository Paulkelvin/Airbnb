...
**Status as of 2026-07-16:** Core platform feature development is complete (Phase 10 — Notifications was the last core module named in `docs/architecture/platform-architecture-blueprint.md` §17). The **Release Hardening milestone** has since closed every **BLOCKER** below that doesn't require an external credential or a live deployment (see ADR-027 and `docs/project-status.md`'s Release Hardening milestone entry for full detail) — those items are marked **✅ done** in place, not removed, so the original scope stays visible. A real first Vercel deployment was then attempted against `main` (now the repo's default branch) and failed twice on build-time bugs — both fixed in code and locally verified (**First Deployment Fixes**, ADR-028). A client bug report against that live deployment then surfaced a real, previously undiscovered blocker — **mobile users could not book at all**, now fixed (**Mobile UX Pass**, ADR-029). **Neon database migrations are now fully applied** — all 10 migrations deployed via a `vercel-build` script that runs `prisma migrate deploy && prisma db seed && next build` on every Vercel deploy; `postgis`/`pgcrypto` extensions enabled; reference data seeded; live site search and amenity filters confirmed working. **Vercel deployment is green and the site is live.** Remaining open items below are Cloudinary, Stripe, and Resend credential provisioning plus final verification steps. This document replaces "next phase" planning in `docs/project-status.md` and is the canonical checklist from here to launch. Update it as items are completed — do not let it drift the way `project-status.md`'s own working agreement forbids for that file.
...
|---|---|---|---|
| Neon (Postgres) | ✅ Connected and migrated | **BLOCKER** — resolved | Real `DATABASE_URL`/`DATABASE_URL_UNPOOLED` set in Vercel. All 10 migrations applied, `postgis`/`pgcrypto` enabled, reference data seeded. Live site search and amenity filters confirmed working. |
...
| Vercel | ✅ Deployed and live | **BLOCKER** — resolved | Build succeeds with `vercel-build` script (`prisma migrate deploy && prisma db seed && next build`). Live site confirmed working. |
...
- [ ] **ENHANCEMENT** — Bundle-size audit / code-splitting review now that the app has grown to 43 routes across 10 feature areas.
