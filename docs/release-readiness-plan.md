...
- [ ] **CRITICAL (`project-status.md` §10, C1)** — `prisma/seed.ts` creates a demo admin account (full `ADMIN` role) and a demo host account, both with plaintext passwords committed to source control (see the script for the actual values — intentionally not repeated here or in any other document). This script has been run against the real production database. Anyone with read access to this repository can log into the live admin dashboard today. **Client decision (2026-07-18): credentials are not being rotated yet** (a private copy is held outside this repo) — this pass removed the values from all public-facing docs/comments; rotation remains a follow-up the client will schedule separately. See `prisma/seed.ts`'s header comment for the recommended long-term approach (environment-variable-driven seed credentials).
...
- [x] **`project-status.md` §10, C8 — resolved 2026-07-18.** Was 21 known vulnerabilities (12 moderate, 9 high); root cause was `package.json`'s Yarn-only `resolutions` block never applying to the project's npm install path. Fixed with a matching `overrides` block plus a few more pins — down to 2 (1 moderate, 1 high), both inside Next.js's own dependency tree and only resolvable by the major-version upgrade already tracked as its own HIGH item above (C2).
...
- [ ] **ENHANCEMENT** — Bundle-size audit / code-splitting review now that the app has grown to 57 routes. The orphaned `/studio` route, previously the single largest bundle in the app at ~1.7 MB, was removed 2026-07-18 (`project-status.md` §10 C4) — this item now covers the remaining app routes only.
...
- [ ] A decision made (and recorded here) on `project-status.md` §10 findings C3 (whether `vercel-build` should re-seed reference data automatically) and C5 (provision a real separate test/staging database — a code-level guard against tests hitting production was added 2026-07-18, but no second database has been provisioned). **C4 (orphaned `/studio` route) resolved 2026-07-18 — route removed.**
