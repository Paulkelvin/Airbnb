...
| Vercel | Deployment attempted, not green yet | **BLOCKER** | First real build attempt failed twice (missing `postinstall: prisma generate`; `sitemap.ts` querying the DB at build time) — both fixed in code and verified locally (ADR-028), but the deployment hasn't been re-run since, and no real `DATABASE_URL`/other credentials are set in the Vercel project — the next attempt will very likely still fail (or succeed but 500 at runtime) purely on missing env vars. Re-run the deployment once credentials below are set. |
