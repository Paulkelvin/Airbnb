...
## ADR-029: Mobile UX Pass — Icon Font Cleanup, Swipe Scroll-Jump Fix, Real Mobile Date Pickers, Real Mobile Booking

**Decision:** Four fixes triggered by client-reported bugs against the live deployment, found together during one investigation pass:

1. **Every remaining Line Awesome (`las`/`lab la-*`) icon reference replaced with Heroicons or small hand-drawn SVGs.** Phase 1 removed the Line Awesome font/CSS as dead-weight cleanup but never audited for lingering `<i className="las la-...">` usages — 8 files still referenced glyphs from a font that no longer ships, rendering as invisible/empty icons (most visibly: the "Suggestions for discovery" carousel's prev/next arrows, reported by the client as "doesn't even have icon"). Heroicons (already the project's only icon dependency) covers most cases (`ChevronLeftIcon`/`ChevronRightIcon`/`ChevronDownIcon`/`CheckIcon`/`UserIcon`/`NoSymbolIcon`/`HomeIcon`/`MapIcon`); three glyphs Heroicons doesn't ship (bed, bath, wifi) and three social brand marks (Facebook/Twitter/Instagram) got small hand-drawn SVGs in a new `src/components/ui/MiscIcons.tsx`, matching Heroicons' own stroke/viewBox conventions so they sit visually consistent.

2. **`useSwipeable`'s `preventScrollOnSwipe: true` added to all three sliders that use it** (`SectionSliderNewCategories.tsx`, `GallerySlider.tsx`, `listing-image-gallery/SharedModal.tsx`). Without it, a horizontal swipe on a touch device also lets the browser's native vertical scroll engage simultaneously — reported by the client as the page "moves up or down a little" while sliding. This is react-swipeable's own documented option for exactly this interaction, not a custom fix.

3. **The three real `StayDatesRangeInput`/`DatesRangeInput` components (desktop `HeroSearchForm`, desktop `HeroSearchFormSmall`, mobile `HeroSearchForm2Mobile`) had each independently hard-coded `new Date("2023/02/06")`/`new Date("2023/02/23")` as their initial state** — a stale demo default from the original template, never replaced with `null`. The mobile one was worse than cosmetic: its `onChange` never called back up to its parent (`StaySearchForm`, which held its *own*, separately-hardcoded, equally-disconnected copy of the same dates) — no date a mobile user ever picked reached the actual search request. All three now start at `null` (showing "Add dates"/"Where to?" correctly on first load); the mobile one is now a fully controlled component (`startDate`/`endDate`/`onChange` props) with no internal state of its own. The mobile picker also now collapses to 1 month below the `sm` breakpoint (was hard-coded to 2, cramped on a phone screen and the direct subject of the client's "minimize on mobile" request) — `DatePickerCustomHeaderTwoMonth.tsx` was generalized to accept a `monthsShown` prop instead of assuming exactly 2 (previous hide/show-arrow logic was hard-coded for a 2-month layout and would have hidden the "next" arrow entirely on a collapsed single month).

4. **`MobileFooterSticky`/`ModalReserveMobile`/the `checkout/PageMain` chain — a real, previously undiscovered bug: mobile users could not book at all.** `BookingWidget` (Phase 5's real, correctly-wired booking form, calling `createShortTermBooking`/`createLongTermBooking`) is `hidden lg:block` — desktop only. The layout-level `MobileFooterSticky` that mobile saw instead showed a hard-coded fake `$311/night` (not the real listing's price — the layout can't access page-level listing data) and its "Reserve" button opened `ModalReserveMobile` → the orphaned template demo `checkout/PageMain`, which is not linked from anywhere in the real app and calls no real booking action. Fixed by deleting both dead components from the layout and building `MobileBookingBar.tsx` (listing-detail-page-scoped, so it has the real listing's price/currency/pricing) that shows the real price and opens the *same* `BookingWidget` component inside a bottom-sheet `Dialog` — no duplicated booking logic, one real form for both breakpoints. Also fixed a resulting layout collision: the site's global `FooterNav` (Explore/Wishlists/Log in/Menu, rendered in the root layout on every page) is also fixed to the mobile viewport bottom — `MobileBookingBar` sits at `bottom-14` (FooterNav's measured 56px height), not `bottom-0`, so the two don't overlap.

**Reasoning:** All four were found investigating the client's own bug reports (a listing-page crash turned out to be a Neon migration gap — see project-status.md — surfaced alongside these UI issues in the same session) plus direct visual inspection of the flagged carousel/date-picker screenshots against a real inspiration reference the client provided. None are speculative — each has a concrete before/after reproduction.

**Alternatives Considered:** (1) For the mobile booking fix: keep `MobileFooterSticky` as a separate component and thread the real listing's pricing data down to it as new props, rather than removing it and reusing `BookingWidget`. (2) For the date pickers: leave `monthsShown={2}` on mobile and only fix the disconnected-state bug.

**Why Rejected:** (1) Client's own explicit direction, offered a choice between the two approaches (`docs/project-status.md` records the exact question asked and answered) — reusing `BookingWidget` means one real implementation of booking logic instead of two, so the fix can't regress into "fake again" the next time someone edits pricing display without knowing a second copy exists. (2) Two months at ~180px each doesn't fit a ~390px mobile viewport without significant horizontal scroll/cramping — the client's own report named this explicitly ("minimized... maybe like a slide"); the header's existing prev/next navigation already gives month-to-month movement once collapsed to one, so nothing needed rebuilding beyond generalizing the header's arrow-visibility logic.

**Trade-offs Accepted:** The three social-brand SVGs in `MiscIcons.tsx` are simplified hand-drawn approximations of the Facebook/Twitter/Instagram marks, not the vendors' official logo assets — acceptable since `SocialsList`'s hrefs are still `#` placeholders pending real Potomac social URLs (Pre-Phase-9 branding audit), so brand fidelity on these icons isn't yet load-bearing. `checkout/PageMain.tsx` and its remaining dependents (`ModalSelectDate.tsx`, `SectionDateRange.tsx`) were left in place rather than deleted — confirmed unreferenced from anywhere in the real app, but removing an entire orphaned route is a bigger, more visible change than this pass's scope; flagged in `docs/project-status.md`'s Known Issues instead.

**Revisit If:** Real Potomac social media URLs and a brand-asset export become available — swap `MiscIcons.tsx`'s Facebook/Twitter/Instagram paths for the official marks. If `checkout/`/`ModalSelectDate.tsx`/`SectionDateRange.tsx` are confirmed still unreferenced in a future pass, delete them outright per the project's established dead-code-removal precedent (Phase 1).
...
| 029 | Mobile UX pass — icon font cleanup, swipe scroll-jump fix, real mobile date pickers, real mobile booking |
