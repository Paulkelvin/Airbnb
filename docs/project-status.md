...
- **Current phase:** Phase 7 (Messaging) complete and approved. Phase 8 (Reviews and Favorites) not yet started.
- **Current branch:** `claude/booking-platform-overhaul-j2unm6`
- **Latest commit:** `cec4063` — "Add Phase 7 Messaging: conversations, threads, and the admin dispute path" (2026-07-15)
- **Build status:** ✅ Clean. `npx next build` succeeds with no errors (27 routes, all typed routes resolve). Last verified 2026-07-15 at the end of Phase 7.
- **Test status:** ✅ Clean. `npm test` (Vitest): 30/30 passing across 3 files. `node_modules/.bin/tsc --noEmit`: zero errors. Last verified 2026-07-15.
...
Per `docs/architecture/platform-architecture-blueprint.md` §17 (the authoritative sequencing, re-checked before Phase 7 began):

1. **Reviews and Favorites** *(next)* — two-sided reviews (`Review.direction`, guest→host and host→guest, one row per direction, double-blind visibility until both submit or a window closes), `Favorite` (saved listings — the existing `LikeSaveBtns` UI component is currently unwired demo UI).
2. **Admin Dashboard** — listing moderation queue, user management (suspend/verify), booking/dispute oversight (surfacing the `DISPUTED` bookings and `CHARGEBACK` payments Phase 6 already produces), manual refunds/adjustments, category/amenity taxonomy management, platform-wide analytics, audit log viewer.
3. **Notifications** — email delivery + in-app preferences UI, **and** the emission primitive itself. The blueprint's roadmap describes a minimal `notify()` primitive (writes a `Notification` row, no delivery) as shared infrastructure meant to exist since the auth phase so later phases could call it — confirmed on 2026-07-15 that **no such function exists anywhere in the codebase yet** (verified via full-codebase search). The `Notification`/`NotificationPreference` tables exist in the schema (unused, zero rows written to them by any code path so far) but this phase needs to build the emission primitive from scratch, not just the delivery/UI layer on top of one that already exists.
4. **Performance Optimization** — load testing and query tuning under real traffic. Per the blueprint, explicitly **not** a catch-up phase for indexes that should already exist (they were front-loaded in Phase 2).
5. **Final Testing and Production Readiness** — the last blueprint phase; real-credential verification (see Infrastructure below) belongs here at the latest, likely earlier once the client supplies credentials.
...
No other unresolved business-rule gaps are currently known. If Reviews (double-blind window length, minimum stay-completion requirement before reviewing) or Admin (dispute-resolution authority limits) surface undocumented business rules, stop and ask per standing instruction — do not assume.
...
**Reviews and Favorites** (blueprint step 10). Prerequisites: none outstanding — `Review` and `Favorite` models already exist in the schema (Phase 2), `Booking.status = COMPLETED` already exists as the natural review-eligibility trigger (Phase 5). Before writing code:
1. Re-read `docs/architecture/domain-model-specification.md` §2.10 (Review) and §2.11 (Favorite) in full.
2. Confirm the double-blind review visibility window mechanism (spec should state whether it's "both submitted" or "N days after checkout, whichever first" — if genuinely unstated, this is a business-rule gap requiring a clarifying question before implementation, not an assumption).
3. Check whether `LikeSaveBtns` (existing but unwired UI component, referenced from `ListingDetailView`) is meant to be reused as-is for Favorites or rebuilt.
...
**Where things are:**
- Domain/business logic: `src/modules/<domain>/{actions.ts,queries.ts}` (`bookings`, `listings`, `inquiries`, `messaging`, `payments`).
