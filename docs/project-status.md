...
**Last updated:** 2026-07-17, end of a documentation-freeze pass (this session did no feature work — see §1).
...
- **Latest commit:** `ec8f5c3` — "Listing review by default, calendar UX polish, better cities copy."
- **Working tree:** Clean, fully pushed to origin. No uncommitted changes.
- **Build status:** ✅ Clean. Verified twice this session: once against the live `DATABASE_URL` (a real, provisioned Neon instance — see §4) and once by standing up a disposable local Postgres 16 + PostGIS + pgcrypto database, running all 11 migrations against it from scratch, and building against that. Both succeeded with no errors. 58 routes total (up from 43 at the last doc update — see §2 for what was added). One caveat specific to *this sandbox*: `next build` fails here with `TypeError: Invalid URL` unless `NEXTAUTH_URL` is corrected first — the injected value in this container has a stray leading space, which trips a strict `url.startsWith("http")` check inside `next-auth`'s own `parseUrl` helper. This was confirmed to be a **sandbox environment artifact, not a code defect**: the identical failure reproduces on the untouched base branch with no changes applied, and disappears the moment the env var is trimmed. Not a launch blocker; worth a sanity-check that the real Vercel env var has no leading/trailing whitespace.
- **Test status:** ✅ Clean, with the same caveat. `npx tsc --noEmit`: zero errors. `npm test` (Vitest): **101/101 passing across 16 files** when run against a reachable database. In this sandbox specifically, `DATABASE_URL` points directly at the real production Neon instance (see §10, finding C5) and the sandbox's network policy only proxies HTTPS/443, not raw Postgres/5432 — so 13 of the 16 DB-integration test files fail here with connection-refused errors, not logic failures. This was verified conclusively by standing up a disposable local database (see Build status above) and re-running the full suite against it: **101/101 passed, 0 failures.**
...
| Content / CMS | **Sanity** (`@sanity/client`, `sanity`, `next-sanity`, `@sanity/image-url`, `@portabletext/react`) for blog posts, standalone pages, FAQ, and the About page — edited through a custom in-admin editor (§2.18), *not* Sanity's own Studio UI (the Studio still exists at `/studio` but is orphaned — see §10, finding C4) |
...
- **Empty scaffold module folders remain from Phase 1:** `src/modules/booking` (singular, empty — real code is in `src/modules/bookings`), `src/modules/auth`, `src/modules/search`, `src/modules/users`. Don't create new files there.
- **`src/app/checkout/{page,PageMain}.tsx`, `src/components/ModalSelectDate.tsx`, `src/app/(listing-detail)/SectionDateRange.tsx` are confirmed orphaned template dead code**, unreachable except by typing the URL directly. Safe to delete in a future cleanup pass.
...
- **C4 — orphaned `/studio` Sanity Studio route still deployed**, ~1.7 MB bundle, no app-level auth gate (MEDIUM).
- **C5 — no dedicated test/staging database**; the project's own integration-test suite would run directly against production data if executed from a machine with full Neon connectivity (MEDIUM).
- **C8 — `npm audit`: 21 vulnerabilities (12 moderate, 9 high)**, up from the previously-documented 18 — not individually triaged (LOW/ENHANCEMENT, separate from C2's Next.js-specific subset).
...
1. **Resolve finding C1** — hardcoded demo admin/host credentials. At minimum: rotate both passwords, and decide whether these demo accounts should exist in production at all (they may be intentional for client demos — confirm, don't just delete unilaterally).
...
**C4 — MEDIUM — The orphaned `/studio` Sanity Studio route is still deployed, unlinked, and un-gated.**
`src/app/studio/[[...tool]]/page.tsx` and its layout still exist and are still reachable — confirmed no auth check exists at the Next.js layer (`StudioLayout` is a pure passthrough; access to actually mutate content relies entirely on Sanity's own external authentication, not this app's `requireAdmin()`). Per the build output, it's also the single largest bundle in the entire application (~1.7 MB, dwarfing every other route). This route was explicitly superseded this session (§2.18) after direct client instruction that Sanity's own UI was not the intended editing surface — but it was never removed. Two divergent content-editing paths for the same Sanity dataset now exist (the audited, `requireAdmin()`-gated `/admin/content/*` actions, and raw Sanity Studio for anyone with Sanity-side collaborator access). Recommend removing the route, or at minimum confirming no one still has a reason to use it before deleting.

**C5 — MEDIUM — No dedicated test/staging database; the integration-test suite targets the production connection string.**
This session's environment had `DATABASE_URL`/`DATABASE_URL_UNPOOLED` pointed directly at the same Neon instance referenced throughout this project's history as "the" production database. The project's own Vitest suite includes real DB-writing integration tests (users, bookings, messages, rate-limit hits, etc.) — anyone running `npm test` from a machine with full network access to Neon would be writing test data directly into production. This sandbox's network policy (HTTPS-only, no raw Postgres) accidentally prevented that from happening here, which is not the same thing as it being prevented by design. Recommend provisioning a separate Neon branch (or any reachable Postgres) specifically for local/CI test runs, and updating `.env`/CI configuration to point there instead.
...
**C8 — LOW/ENHANCEMENT.** `npm audit`: 21 vulnerabilities (12 moderate, 9 high) — up from the previously-documented 18 (9 moderate, 9 high), driven mostly by Sanity's dependency tree (`@sanity/uuid`/`@sanity/preview-url-secret` pulling in a vulnerable `uuid` range). Not individually triaged. The Next.js-specific subset is broken out separately as C2 because of its severity; the rest remains an accepted, un-urgent backlog item exactly as it was before.
