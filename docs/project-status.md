...
Last updated: **2026-07-16**, First Deployment Fixes complete (see below — ADR-028) on top of the Release Hardening milestone (security headers, rate limiting extension, password-reset delivery, SEO metadata). Phase 10 (Notifications) was the last core platform module; the project is in release readiness. See `docs/release-readiness-plan.md` for what's left (all of it now infrastructure/credentials-gated, not code).
...
- **Current phase:** First Deployment Fixes complete on top of the Release Hardening milestone — the release-readiness plan's entire "code-only" bucket (§1, §4, §7) is closed, and the first real Vercel deployment attempt's two build failures are fixed. Everything remaining in `docs/release-readiness-plan.md` requires external credentials or further live-deployment verification (Neon, Cloudinary, Stripe, Resend) and is explicitly out of scope until those are provisioned.
- **Current branch:** `claude/booking-platform-overhaul-j2unm6` (also pushed to `main`, now the repo's default branch)
- **Latest commit:** (see git log)
- **Build status:** ✅ Clean. `npx next build` succeeds with no errors (43 routes total, unchanged route set) — verified both with local Postgres running and, critically, with it **stopped entirely** (confirms the build no longer has any database dependency, per ADR-028). Last verified 2026-07-16.
- **Test status:** ✅ Clean. `npm test` (Vitest): 101/101 passing across 16 files (added `src/actions/__tests__/forgot-password.test.ts` — full forgot→email→reset round trip against the real emailed token, plus rate-limit exhaustion; `src/lib/__tests__/auth-options-login-rate-limit.test.ts` — exercises the NextAuth credentials provider's `authorize()` directly against the real DB; fixed a pre-existing test-hygiene gap in `src/modules/notifications/__tests__/retrofit-integration.test.ts`, see Bugs found/fixed below). `npx tsc --noEmit`: zero errors. Also verified live in a real browser (temporary Playwright pass, per this project's established pattern): signup → header shows real session → log out actually signs out → header reverts to "Log in" → forgot-password → stub-emailed reset link → reset-password page (missing-token and invalid-token states) → login rate-limit message after repeated failed attempts.
...
**There is no next feature phase.** Core platform functionality is complete as of Phase 10 (Notifications), and the Release Hardening milestone has closed every code-only blocker (security headers, rate limiting, password-reset delivery, header/session wiring, SEO metadata — see ADR-027). Everything remaining in **`docs/release-readiness-plan.md`** requires external credentials or a live deployment: production infrastructure provisioning (Neon, Cloudinary, Stripe, Resend, Vercel), deployment/webhook verification, load testing, accessibility review (screen reader / keyboard nav passes need a real browser session against real content), monitoring/observability setup, backup/recovery verification, and the final cross-browser golden-path E2E walkthrough against the real deployed stack — each item marked as a launch blocker or a safe-to-ship-after-launch enhancement. Work resumes once those credentials are provisioned.
