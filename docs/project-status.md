...
- **C5 — no dedicated test/staging database**; the project's own integration-test suite would run directly against production data if executed from a machine with full Neon connectivity (MEDIUM). **Partially mitigated 2026-07-18** — see §2.23: the test runner now refuses to run against anything it can identify as production and warns loudly otherwise, but provisioning an actual separate database is still an open manual step.
